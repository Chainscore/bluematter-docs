...
- **`_decode_conway_block(block_data_raw, envelope_cbor)`** (line 139): Internal Conway decode. Walks the 5-element block array `[header, tx_bodies, tx_witnesses, auxiliary_data, invalid_txs]`, validates structural invariants, decodes all components from byte-sliced raw data.
